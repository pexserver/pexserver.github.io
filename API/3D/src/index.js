class StereoscopicViewer {
    constructor() {
        ...
        this._resizeViewerHandler = () => this.resizeViewer();
        this.canvas = document.getElementById('anaglyphCanvas');
        this.ctx = this.canvas.getContext('2d');
        this.videoElement = document.getElementById('anaglyphVideo');
    ...
    addVideoControls() {
        // すでに追加済みなら何もしない
        if (document.getElementById('videoControlBar'))
            return;
    ...
    playAnaglyphVideo() {
        ...
        const render = () => {
            if (this.mediaType !== 'video' || this.video.paused || this.video.ended)
                return;
    ...
    handleFullscreenResize() {
        ...
        if (!viewer)
            return;
        // フルスクリーン中かどうかを判定
        const isFullscreen = document.fullscreenElement === viewer || document.webkitFullscreenElement === viewer;
        if (isFullscreen) {
            // サイズを最大化
