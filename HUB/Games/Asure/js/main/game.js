...
window.addEventListener('keydown', e => {
    ...
    // グローバル抑止フラグが立っているときも活性化を防ぐ（オーバーレイ表示直後のキーリピート等）
    const g = (typeof window !== 'undefined' && window.__ASURE__) ? window.__ASURE__ : null;
    if ((overlayVisible || (g && g.suppressActivate)) && isActivateKey) { e.preventDefault(); return; }
...
function isJumpPressed() {
    ...
    if (typeof window !== 'undefined' && window.__ASURE__ && window.__ASURE__.inputLock > 0) return false;
    return keys.has(' ') || keys.has('arrowup') || keys.has('w');
}
function axis() {
    if (anyOverlayVisible()) return 0;
    if (typeof window !== 'undefined' && window.__ASURE__ && window.__ASURE__.inputLock > 0) return 0;
    return (keys.has('arrowleft') || keys.has('a') ? -1 : 0) + (keys.has('arrowright') || keys.has('d') ? 1 : 0);
}
...
class Game {
    ...
    loadStage(idx) {
        ...
        // キー状態クリアと短時間入力ロックをかけて、ロード直後の誤入力を防ぐ
        try { keys.clear(); } catch (e) { }
        this.mobileLeft = false; this.mobileRight = false; this.mobileJump = false;
        this.inputLock = 0.18; // 秒
    ...
    nextStage() {
        // 抑止をセットして、直後の Space/Enter で再度発火しないようにする
        try { if (typeof window !== 'undefined' && window.__ASURE__) window.__ASURE__.suppressActivate = true; } catch (e) {}
        ...
        if (this.stageIndex + 1 >= CONFIG.stages) {
            ...
            this.overlayStart.classList.remove('visible');
            // リザルトは選択式にするため started を false にして再スタート可能にする
            this.started = false;
            // ほんの少し遅延して抑止を解除
            setTimeout(() => { try { if (typeof window !== 'undefined' && window.__ASURE__) window.__ASURE__.suppressActivate = false; } catch (e) {} }, 300);
            return;
        }
        // 次ステージロード前に音や再生ループ類をリセットし、確実にGC
        if (this.bgmPlaying) { this.bgmPlaying = false; }
        this.loadStage(this.stageIndex + 1);
        // 次ステージ遷移直後も短時間抑止（ボタンクリック連打防止）
        setTimeout(() => { try { if (typeof window !== 'undefined' && window.__ASURE__) window.__ASURE__.suppressActivate = false; } catch (e) {} }, 300);
    }
    ...
    logic(dt) {
        ...
        if (p.mesh.position.x > this.stageLength + 2) {
            ...
            // オーバーレイ表示直後の Space / Enter による誤トリガを防止
            try { if (typeof window !== 'undefined') { window.__ASURE__ = window.__ASURE__ || this; window.__ASURE__.suppressActivate = true; } } catch (e) {}
            setTimeout(() => { try { if (typeof window !== 'undefined' && window.__ASURE__) window.__ASURE__.suppressActivate = false; } catch (e) {} }, 300);
            this.playSE('clear');
        }
